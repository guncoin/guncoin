...

    {
        ...
        while (it != mapMasternodes.end()) {
            ...
            if (it->second.IsOutpointSpent()) {
                LogPrint(BCLog::MASTERNODE, "CMasternodeMan::%s -- Removing Masternode: %s  addr=%s  %i now\n", __func__, it->second.GetStateString(), it->second.addr.ToString(), size() - 1);
                ...
                if(fAsk) {
                    ...
                    if(fAskedForMnbRecovery) {
                        LogPrint(BCLog::MASTERNODE, "CMasternodeMan::%s -- Recovery initiated, masternode=%s\n", __func__, it->first.ToStringShort());
                        nAskForMnbRecovery--;
                    }
        ...
        LogPrint(BCLog::MASTERNODE, "CMasternodeMan::%s -- mMnbRecoveryGoodReplies size=%d\n", __func__, (int)mMnbRecoveryGoodReplies.size());
        ...
        while(itMnbReplies != mMnbRecoveryGoodReplies.end()){
            if(mMnbRecoveryRequests[itMnbReplies->first].first < GetTime()) {
                ...
                if(itMnbReplies->second.size() >= MNB_RECOVERY_QUORUM_REQUIRED) {
                    ...
                    LogPrint(BCLog::MASTERNODE, "CMasternodeMan::%s -- reprocessing mnb, masternode=%s\n", __func__, itMnbReplies->second[0].outpoint.ToStringShort());
                ...
                LogPrint(BCLog::MASTERNODE, "CMasternodeMan::%s -- removing mnb recovery reply, masternode=%s, size=%d\n", __func__, itMnbReplies->second[0].outpoint.ToStringShort(), (int)itMnbReplies->second.size());
    ...
    }
    {
        ...
        while(it4 != mapSeenMasternodePing.end()){
            if((*it4).second.IsExpired()) {
                LogPrint(BCLog::MASTERNODE, "CMasternodeMan::%s -- Removing expired Masternode ping: hash=%s\n", __func__, (*it4).second.GetHash().ToString());
        ...
        while(itv2 != mapSeenMasternodeVerification.end()){
            if((*itv2).second.nBlockHeight < nCachedBlockHeight - MAX_POSE_BLOCKS){
                LogPrint(BCLog::MASTERNODE, "CMasternodeMan::%s -- Removing expired Masternode verification: hash=%s\n", __func__, (*itv2).first.ToString());
...
masternode_info_t CMasternodeMan::FindRandomNotInVec(const std::vector<COutPoint> &vecToExclude, int nProtocolVersion)
{
    ...
    FastRandomContextDash insecure_rand;
    // shuffle pointers
...
void CMasternodeMan::ProcessPendingMnbRequests(CConnman& connman)
{
    ...
    LogPrint(BCLog::MASTERNODE, "%s -- mapPendingMNB size: %d\n", __func__, mapPendingMNB.size());
}
...
void CMasternodeMan::ProcessPendingMnvRequests(CConnman& connman)
{
    ...
    LogPrint(BCLog::MASTERNODE, "%s -- mapPendingMNV size: %d\n", __func__, mapPendingMNV.size());
}
...
void CMasternodeMan::UpdateLastPaid(const CBlockIndex* pindex)
{
    ...
    LogPrint(BCLog::MASTERNODE, "CMasternodeMan::%s -- nCachedBlockHeight=%d, nLastRunBlockHeight=%d, nMaxBlocksToScanBack=%d\n",
                            __func__, nCachedBlockHeight, nLastRunBlockHeight, nMaxBlocksToScanBack);
