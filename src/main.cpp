...
#if defined(NDEBUG)
# error "Guncoin cannot be compiled without assertions."
#endif
...
#define DIFF_CHANGE_DIGISHIELD_TESTNET 30
#define DIFF_CHANGE_DIGISHIELD 30
...
const string strMessageMagic = "Guncoin Signed Message:\n";
...
{
    // Guncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 8 * 60 * 60; // Guncoin: 8 hours
static const int64 nTargetSpacing = 2 * 60; // Guncoin: 2 minutes
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock){
    ...
    // Guncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    int DiffMode = 1;
    if (fTestNet) {
        if (pindexLast->nHeight+1 >= DIFF_CHANGE_DIGISHIELD_TESTNET) { DiffMode = 2; }
    }
    else {
        if (pindexLast->nHeight+1 >= DIFF_CHANGE_DIGISHIELD) { DiffMode = 2; }
    }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Guncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Guncoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x765d9ec322c494d36d36f300c209f13b787b6f67bf978c5b9ab09bc9e10f52b8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1397923502;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085244361;
        ...
        if (fTestNet)
        {
            block.nTime    = 1397925814;
            block.nNonce   = 385915966;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9279661f2031143951417ef97ea70bcd7172f1bff7831c339c97e0e8392a926a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xaa, 0xc3, 0xc6, 0xab }; // Guncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GuncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GuncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GuncoinMiner(CWallet *pwallet)
{
    printf("GuncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("guncoin-miner");
