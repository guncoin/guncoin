...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "start-alias")
    {
        ...
        for (const auto& mne : masternodeConfig.getEntries()) {
            if(mne.getAlias() == strAlias) {
                ...
                bool fResult = CMasternodeBroadcast::Create(*iwallet, mne.getIp(), mne.getPrivKey(), mne.getTxHash(), mne.getOutputIndex(), strError, mnb);
    ...
    if (strCommand == "outputs") {
        ...
        pwallet->AvailableCoins(vPossibleCoins, true, NULL, 1, MAX_MONEY, MAX_MONEY, 0, 0, 9999999, ONLY_200000);
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
    ...
    if (strCommand == "create-alias")
    {
        ...
        for (const auto& mne : masternodeConfig.getEntries()) {
            if(mne.getAlias() == strAlias) {
                ...
                bool fResult = CMasternodeBroadcast::Create(*iwallet, mne.getIp(), mne.getPrivKey(), mne.getTxHash(), mne.getOutputIndex(), strError, mnb, true);
