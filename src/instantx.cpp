...
{
    if(fLiteMode)
        return; // disable all Masternode specific functionality
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest, CConnman& connman)
{
    ...
    LOCK(wallets[0] ? &wallets[0]->cs_wallet : NULL);
#endif
...
bool CInstantSend::ProcessOrphanTxLockVote(const CTxLockVote& vote)
{
    ...
    std::vector<std::shared_ptr<CWallet>> wallets = GetWallets();
    if (wallets[0])
        AssertLockHeld(wallets[0]->cs_wallet);
#endif
    AssertLockHeld(cs_instantsend);
...
#ifdef ENABLE_WALLET
    if(wallets[0] && wallets[0]->UpdatedTransaction(txHash)) {
...
bool CTxLockRequest::IsValid() const
{
    ...
    if(nValueIn > INSTANTSEND_MAX_VALUE) {
...
{
    connman.RelayTransaction(txLockRequest.tx->GetHash());
