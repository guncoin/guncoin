...
void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet, CTxOut& txoutDeveloperRet)
{
    mnpayments.FillBlockPayee(txNew, nBlockHeight, blockReward, txoutMasternodeRet, txoutDeveloperRet);
    LogPrint(BCLog::MNPAYMENTS, "FillBlockPayments -- nBlockHeight %d blockReward %lld txoutMasternodeRet %s txoutDeveloperRet %s txNew %s",
                            nBlockHeight, blockReward, txoutMasternodeRet.ToString(), txoutDeveloperRet.ToString(), txNew.ToString());
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet, CTxOut& txoutDeveloperRet) const
{
    // make sure it's not filled yet
    txoutMasternodeRet = CTxOut();
    txoutDeveloperRet = CTxOut();

    // Get developer payment setup
    CAmount developerPayment = GetDeveloperPayment(blockReward);

    // Reduce developer payment from network reward
    txNew.vout[0].nValue -= developerPayment;

    txoutDeveloperRet = CTxOut(developerPayment, Params().GetRewardScript());
    txNew.vout.push_back(txoutDeveloperRet);
    ...
    CAmount masternodePayment = GetMasternodePayment(blockReward - developerPayment);
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
...

        {
            ...
            if(!res.second && res.first->second.IsVerified()) {
                LogPrint(BCLog::MNPAYMENTS, "MASTERNODEPAYMENTVOTE -- hash=%s, nBlockHeight=%d/%d seen\n",
...
bool CMasternodePayments::AddOrUpdatePaymentVote(const CMasternodePaymentVote& vote)
{
    ...
    LogPrint(BCLog::MNPAYMENTS, "CMasternodePayments::AddOrUpdatePaymentVote -- added, hash=%s\n", nVoteHash.ToString());
...
bool CMasternodeBlockPayees::HasPayeeWithVotes(const CScript& payeeIn, int nVotesReq) const
{
    ...
    LogPrint(BCLog::MNPAYMENTS, "CMasternodeBlockPayees::%s -- ERROR: couldn't find any payee with %d+ votes\n", __func__, nVotesReq);
    return false;
}
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    CAmount blockReward = txNew.GetValueOut();
    CAmount nMasternodePayment = GetMasternodePayment(blockReward - GetDeveloperPayment(blockReward));
        ...
        if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            for (const auto& txout : txNew.vout) {
                if (payee.GetPayee() == txout.scriptPubKey && nMasternodePayment == txout.nValue) {
                    LogPrint(BCLog::MNPAYMENTS, "CMasternodeBlockPayees::%s -- Found required payment\n", __func__);
                    return true;
                }
            }
    ...
    LogPrintf("CMasternodeBlockPayees::%s -- ERROR: Missing required payment, possible payees: '%s', amount: %f GUN\n", __func__, strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
std::string CMasternodeBlockPayees::GetRequiredPaymentsString() const
{
    ...
    for (const auto& payee : vecPayees)
    {
        ...
        strRequiredPayments += strprintf("%s:%d", EncodeDestination(address1), payee.GetVoteCount());
    }
...
void CMasternodePayments::UpdatedBlockTip(const CBlockIndex *pindex, CConnman& connman)
{
    ...
    LogPrint(BCLog::MNPAYMENTS, "CMasternodePayments::%s -- nCachedBlockHeight=%d\n", __func__, nCachedBlockHeight);
