...
{
    LogPrint(BCLog::MASTERNODE, "CActiveMasternode::%s -- Start\n", __func__);
    if(!fMasternodeMode) {
        LogPrint(BCLog::MASTERNODE, "CActiveMasternode::%s -- Not a masternode, returning\n", __func__);
        return;
    }

    if(Params().NetworkIDString() != CBaseChainParams::REGTEST && !masternodeSync.IsBlockchainSynced()) {
        nState = ACTIVE_MASTERNODE_SYNC_IN_PROCESS;
        LogPrintf("CActiveMasternode::%s -- %s: %s\n", __func__, GetStateString(), GetStatus());
        return;
    }
    ...
    LogPrint(BCLog::MASTERNODE, "CActiveMasternode::%s -- status = %s, type = %s, pinger enabled = %d\n", __func__, GetStatus(), GetTypeString(), fPingerEnabled);
...
void CActiveMasternode::ManageStateInitial(CConnman& connman)
{
    ...
    if (!fListen) {
        ...
        LogPrintf("CActiveMasternode::%s -- %s: %s\n", __func__, GetStateString(), strNotCapableReason);
        return;
    }
    ...
    // Skip on anything other than mainnet
    if(!fFoundLocal && Params().NetworkIDString() == CBaseChainParams::MAIN) {
        nState = ACTIVE_MASTERNODE_NOT_CAPABLE;
    ...
    LogPrint(BCLog::MASTERNODE, "CActiveMasternode::%s -- End status = %s, type = %s, pinger enabled = %d\n", __func__, GetStatus(), GetTypeString(), fPingerEnabled);
}

void CActiveMasternode::ManageStateRemote()
{
    LogPrint(BCLog::MASTERNODE, "CActiveMasternode::%s -- Start status = %s, type = %s, pinger enabled = %d, pubKeyMasternode.GetID() = %s\n",
             __func__, GetStatus(), GetTypeString(), fPingerEnabled, pubKeyMasternode.GetID().ToString());
    ...
    if(mnodeman.GetMasternodeInfo(pubKeyMasternode, infoMn)) {
        ...
        if(Params().NetworkIDString() == CBaseChainParams::MAIN && service != infoMn.addr) {
            nState = ACTIVE_MASTERNODE_NOT_CAPABLE;
        ...
        if(nState != ACTIVE_MASTERNODE_STARTED) {
            LogPrintf("CActiveMasternode::%s -- STARTED!\n", __func__);
