...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CPubKey& pubkey, int& nHeightRet)
{
    ...
    if(coin.out.nValue != 200000 * COIN) {
...
void CMasternode::Check(bool fForce)
{
    ...
    // Works on mainnet only and not the case on regtest.
...
bool CMasternode::IsValidNetAddr(CService addrIn)
{
    ...
    return (Params().NetworkIDString() == CBaseChainParams::REGTEST) || (Params().NetworkIDString() == CBaseChainParams::TESTNET) ||
...
void CMasternode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack)
{
    ...
    LogPrintf("CMasternode::%s -- searching for block with payment to %s\n", __func__, outpoint.ToStringShort());
    ...
    for (int i = 0; BlockReading && BlockReading->nHeight > nBlockLastPaid && i < nMaxBlocksToScanBack; i++) {
            ...
            mnpayments.mapMasternodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2))
        {
            ...
            if(!ReadBlockFromDisk(block, BlockReading, Params().GetConsensus())) {
                LogPrintf("CMasternode::%s shouldn't really happen\n", __func__);
                continue; // shouldn't really happen
            }

            CAmount blockReward = block.vtx[0]->GetValueOut();
            CAmount nMasternodePayment = GetMasternodePayment(blockReward - GetDeveloperPayment(blockReward));

            LogPrintf("CMasternode::%s blockReward %d nMasternodePayment %d\n", __func__, blockReward, nMasternodePayment);

            for (const auto& txout : block.vtx[0]->vout) {
                if(mnpayee == txout.scriptPubKey && nMasternodePayment == txout.nValue) {
                    nBlockLastPaid = BlockReading->nHeight;
                    nTimeLastPaid = BlockReading->nTime;
                    LogPrintf("CMasternode::%s -- searching for block with payment to %s -- found new %d\n", __func__, outpoint.ToStringShort(), nBlockLastPaid);
                    return;
                }
            }
            LogPrintf("CMasternode::%s didn't find anything\n", __func__);
        }

        if (BlockReading->pprev->nHeight < Params().GetConsensus().nMasternodeEnforcePayment || BlockReading->pprev == nullptr)
            break;
    ...
    LogPrint(BCLog::MNPAYMENTS, "CMasternode::%s -- searching for block with payment to %s -- keeping old %d\n", __func__, outpoint.ToStringShort(), nBlockLastPaid);
}

#ifdef ENABLE_WALLET
bool CMasternodeBroadcast::Create(interfaces::Wallet& iWallet, const std::string& strService, const std::string& strKeyMasternode, const std::string& strTxHash, const std::string& strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline)
{
    COutPoint outpoint;
    ...
    if (!iWallet.getMasternodeOutpointAndKeys(outpoint, pubKeyCollateralAddressNew, keyCollateralAddressNew, strTxHash, strOutputIndex))
...
bool CMasternodeBroadcast::Create(const COutPoint& outpoint, const CService& service, const CKey& keyCollateralAddressNew, const CPubKey& pubKeyCollateralAddressNew, const CKey& keyMasternodeNew, const CPubKey& pubKeyMasternodeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    ...
    LogPrint(BCLog::MASTERNODE, "%s: pubKeyCollateralAddressNew: %s, pubKeyMasternodeNew: %s\n", __func__,
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint(BCLog::MASTERNODE, "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 200,000 GUN, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
...
void CMasternodePing::Relay(CConnman& connman)
{
    // Do not relay until fully synced
    if(!masternodeSync.IsSynced()) {
        LogPrint(BCLog::MASTERNODE, "CMasternodePing::%s -- won't relay until fully synced\n", __func__);
        return;
    }
