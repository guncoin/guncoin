...
void CPrivateSendServer::CommitFinalTransaction(CConnman& connman)
{
...

    {
        ...
        if(!lockMain || !AcceptToMemoryPool(mempool, validationState, finalTransaction, NULL, NULL, false, maxTxFee, true))
        {
...
void CPrivateSendServer::ChargeFees(CConnman& connman)
{
    ...
    if(nState == POOL_STATE_ACCEPTING_ENTRIES || nState == POOL_STATE_SIGNING) {
        ...
        if(!AcceptToMemoryPool(mempool, state, vecOffendersCollaterals[0], NULL, NULL, false, maxTxFee)) {
            // should never really happen
            LogPrintf("CPrivateSendServer::ChargeFees -- ERROR: AcceptToMemoryPool failed!\n");
        } else {
            connman.RelayTransaction(vecOffendersCollaterals[0]->GetHash());
        }
    }
}
...
/*
    ...
    allow endless transaction that would bloat Masternode and make it unusable. To
...
void CPrivateSendServer::ChargeRandomFees(CConnman& connman)
{
    ...
    for (const auto& txCollateral : vecSessionCollaterals) {
        ...
        CValidationState state;
        if(!AcceptToMemoryPool(mempool, state, txCollateral, NULL, NULL, false, maxTxFee)) {
            // should never really happen
            LogPrintf("CPrivateSendServer::ChargeRandomFees -- ERROR: AcceptToMemoryPool failed!\n");
        } else {
            connman.RelayTransaction(txCollateral->GetHash());
        }
    }
}
...
void CPrivateSendServer::CheckTimeout(CConnman& connman)
{
    ...
    if(nState != POOL_STATE_IDLE && fTimeout) {
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendServer::%s -- %s timed out (%ds) -- resetting\n", __func__,
...
void CPrivateSendServer::CheckForCompleteQueue(CConnman& connman)
{
    ...
    if(nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendServer::%s -- queue is ready, signing and relaying (%s)\n", __func__, dsq.ToString());
        dsq.Sign();
        dsq.Relay(connman);
    }
}
...
bool CPrivateSendServer::CreateNewSession(const CDarksendAccept& dsa, PoolMessage& nMessageIDRet, CConnman& connman)
{
    ...
    if(nState != POOL_STATE_IDLE) {
        ...
        LogPrintf("CPrivateSendServer::%s -- incompatible mode: nState=%d\n", __func__, nState);
        return false;
    }
    ...
    if(!fUnitTest) {
        ...
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendServer::%s -- signing and relaying new queue: %s\n", __func__, dsq.ToString());
        dsq.Sign();
        dsq.Relay(connman);
        vecDarksendQueue.push_back(dsq);
    }
    ...
    LogPrintf("CPrivateSendServer::%s -- new session created, nSessionID: %d  nSessionDenom: %d (%s)  vecSessionCollaterals.size(): %d\n",
            __func__, nSessionID, nSessionDenom, CPrivateSend::GetDenominationsToString(nSessionDenom), vecSessionCollaterals.size());
...
void CPrivateSendServer::SetState(PoolState nStateNew)
{
    ...
    if(nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS) {
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendServer::%s -- Can't set state to ERROR or SUCCESS as a Masternode. \n", __func__);
        return;
    }

    LogPrintf("CPrivateSendServer::%s -- nState: %d, nStateNew: %d\n", __func__, nState, nStateNew);
    nState = nStateNew;
}
