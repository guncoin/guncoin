...
void CPrivateSendClient::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Masternode related functionality
    ...
    for (const auto& entry : vecEntries) {
        for (const auto& txdsin : entry.vecTxDSIn) {
            ...
            if(nMyInputIndex >= 0) { //might have to do this one input at a time?
                ...
                std::vector<std::shared_ptr<CWallet>> wallets = GetWallets();
                const CKeyStore& keystore = *wallets[0];
...
bool CPrivateSendClient::CheckAutomaticBackup()
{
    ...
    switch(nWalletBackups) {
        case 0:
            ...
            wallets[0]->nKeysLeftSinceAutoBackup = 0; // no backup, no "keys since last backup"
            return false;
        case -1:
    ...
    if(wallets[0]->nKeysLeftSinceAutoBackup < PRIVATESEND_KEYS_THRESHOLD_STOP) {
        // We should never get here via mixing itself but probably smth else is still actively using keypool
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendClient::CheckAutomaticBackup -- Very low number of keys left: %d, no mixing available.\n", wallets[0]->nKeysLeftSinceAutoBackup);
        strAutoDenomResult = strprintf(_("Very low number of keys left: %d") + ", " + _("no mixing available."), wallets[0]->nKeysLeftSinceAutoBackup);
        // It's getting really dangerous, stop mixing
        fEnablePrivateSend = false;
        return false;
    } else if(wallets[0]->nKeysLeftSinceAutoBackup < PRIVATESEND_KEYS_THRESHOLD_WARNING) {
        // Low number of keys left but it's still more or less safe to continue
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendClient::CheckAutomaticBackup -- Very low number of keys left: %d\n", wallets[0]->nKeysLeftSinceAutoBackup);
        strAutoDenomResult = strprintf(_("Very low number of keys left: %d"), wallets[0]->nKeysLeftSinceAutoBackup);
        ...
        if(fCreateAutoBackups) {
            ...
            if(!AutoBackupWallet(wallets[0].get(), "", warningString, errorString)) {
...
{
    if(fMasternodeMode)
        return false; // no client-side mixing on masternodes
    if(!fEnablePrivateSend)
        return false;
    std::vector<std::shared_ptr<CWallet>> wallets = GetWallets();
    if(!wallets[0] || wallets[0]->IsLocked(true))
        return false;
    ...
    if(!fDryRun && wallets[0]->IsLocked(true)) {
        strAutoDenomResult = _("Wallet is locked.");
        return false;
    }

    if(WaitForAnotherBlock()) {
        LogPrintf("CPrivateSendClient::%s -- Last successful PrivateSend action was too recent\n", __func__);
        strAutoDenomResult = _("Last successful PrivateSend action was too recent.");
        return false;
    }

    if(mnodeman.size() == 0) {
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendClient::%s -- No Masternodes detected\n", __func__);
    ...
    if(!wallets[0]->HasCollateralInputs()) {
        // should have some additional amount for them
        nValueMin += CPrivateSend::GetMaxCollateralAmount();
    }

    // including denoms but applying some restrictions
    CAmount nBalanceNeedsAnonymized = wallets[0]->GetNeedsToBeAnonymizedBalance(nValueMin);

    // anonymizable balance is way too small
    if(nBalanceNeedsAnonymized < nValueMin) {
        LogPrintf("CPrivateSendClient::%s -- Not enough funds to anonymize. Balance: %lld Min: %lld\n", __func__, nBalanceNeedsAnonymized, nValueMin);
        strAutoDenomResult = _("Not enough funds to anonymize.");
        return false;
    }

    // excluding denoms
    CAmount nBalanceAnonimizableNonDenom = wallets[0]->GetAnonymizableBalance(true);
    // denoms
    CAmount nBalanceDenominatedConf = wallets[0]->GetDenominatedBalance();
    CAmount nBalanceDenominatedUnconf = wallets[0]->GetDenominatedBalance(true);
    CAmount nBalanceDenominated = nBalanceDenominatedConf + nBalanceDenominatedUnconf;

    LogPrint(BCLog::PRIVATESEND, "CPrivateSendClient::%s -- nValueMin: %f, nBalanceNeedsAnonymized: %f, nBalanceAnonimizableNonDenom: %f, nBalanceDenominatedConf: %f, nBalanceDenominatedUnconf: %f, nBalanceDenominated: %f\n",
             __func__,
    ...
    if(!wallets[0]->HasCollateralInputs())
        return !wallets[0]->HasCollateralInputs(false) && MakeCollateralAmounts(connman);
    ...
    if(nBalanceDenominatedUnconf > 0) {
        LogPrintf("CPrivateSendClient::%s -- Found unconfirmed denominated outputs, will wait till they confirm to continue.\n", __func__);
    ...
    if(txMyCollateral == CMutableTransaction()) {
        if(!wallets[0]->CreateCollateralTransaction(txMyCollateral, strReason)) {
            LogPrintf("CPrivateSendClient::%s -- create collateral error:%s\n", __func__, strReason);
            return false;
        }
    } else {
        if(!CPrivateSend::IsCollateralValid(txMyCollateral)) {
            LogPrintf("CPrivateSendClient::%s -- invalid collateral, recreating...\n", __func__);
            if(!wallets[0]->CreateCollateralTransaction(txMyCollateral, strReason)) {
                LogPrintf("CPrivateSendClient::%s -- create collateral error: %s\n", __func__, strReason);
                return false;
            }
...
bool CPrivateSendClient::JoinExistingQueue(CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    for (auto& dsq : vecDarksendQueue) {
        ...
        if(!mnodeman.GetMasternodeInfo(dsq.masternodeOutpoint, infoMn)) {
            LogPrintf("CPrivateSendClient::%s -- dsq masternode is not in masternode list, masternode=%s\n", __func__, dsq.masternodeOutpoint.ToStringShort());
            continue;
        }
        ...
        if (mnpayments.IsScheduled(infoMn, 0)) {
            LogPrintf("CPrivateSendClient::%s -- skipping winner, masternode=%s\n", __func__, infoMn.outpoint.ToStringShort());
            continue;
        }
        ...
        if(!CPrivateSend::GetDenominationsBits(dsq.nDenom, vecBits)) {
            // incompatible denom
            LogPrintf("CPrivateSendClient::%s incompatible denom\n", __func__);
            continue;
        }
        ...
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendClient::%s -- found valid queue: %s\n", __func__, dsq.ToString());
        ...
        if(!wallets[0]->SelectCoinsByDenominations(dsq.nDenom, nMinAmount, nMaxAmount, vecTxDSInTmp, vCoinsTmp, nValueInTmp, 0, nPrivateSendRounds)) {
            LogPrintf("CPrivateSendClient::%s -- Couldn't match %d denominations %d (%s)\n", __func__, vecBits.front(), dsq.nDenom, CPrivateSend::GetDenominationsToString(dsq.nDenom));
            continue;
        }
        ...
        if (connman.IsMasternodeOrDisconnectRequested(infoMn.addr)) {
            LogPrintf("CPrivateSendClient::%s -- skipping masternode connection, addr=%s\n", __func__, infoMn.addr.ToString());
            continue;
        }
        ...
        nTimeLastSuccessfulStep = GetTime();
        LogPrintf("CPrivateSendClient::%s -- pending connection (from queue): nSessionDenom: %d (%s), addr=%s\n",
                  __func__, nSessionDenom, CPrivateSend::GetDenominationsToString(nSessionDenom), infoMn.addr.ToString());
...
bool CPrivateSendClient::StartNewQueue(CAmount nValueMin, CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    CAmount nValueInTmp = 0;
    std::vector<std::shared_ptr<CWallet>> wallets = GetWallets();
    if(!wallets[0]->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vecTxIn, nValueInTmp, 0, nPrivateSendRounds)) {
        // this should never happen
        LogPrintf("CPrivateSendClient::%s -- Can't mix: no compatible inputs found!\n", __func__);
    ...
    while(nTries < 10) {
        ...
        if(!infoMn.fInfoValid) {
            LogPrintf("CPrivateSendClient::%s -- Can't find random masternode!\n", __func__);
        ...
        // skip next mn payments winners
        if (mnpayments.IsScheduled(infoMn, 0)) {
            LogPrintf("CPrivateSendClient::%s -- skipping winner, masternode=%s\n", infoMn.outpoint.ToStringShort(), __func__);
            nTries++;
            continue;
        }
        ...
        if(infoMn.nLastDsq != 0 && infoMn.nLastDsq + nMnCountEnabled/5 > mnodeman.nDsqCount) {
            LogPrintf("CPrivateSendClient::%s -- Too early to mix on this masternode!"
                        " masternode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
                        __func__, infoMn.outpoint.ToStringShort(), infoMn.addr.ToString(), infoMn.nLastDsq,
        ...
        LogPrintf("CPrivateSendClient::%s -- attempt %d connection to Masternode %s\n", __func__, nTries, infoMn.addr.ToString());

        std::vector<CAmount> vecAmounts;
        wallets[0]->ConvertList(vecTxIn, vecAmounts);
        ...
        SetState(POOL_STATE_QUEUE);
        nTimeLastSuccessfulStep = GetTime();
        LogPrintf("CPrivateSendClient::%s -- pending connection, nSessionDenom: %d (%s), addr=%s\n",
                __func__, nSessionDenom, CPrivateSend::GetDenominationsToString(nSessionDenom), infoMn.addr.ToString());
        strAutoDenomResult = _("Trying to connect...");
        return true;
    }
...
bool CPrivateSendClient::PrepareDenominate(int nMinRounds, int nMaxRounds, std::string& strErrorRet, std::vector<CTxDSIn>& vecTxDSInRet, std::vector<CTxOut>& vecTxOutRet)
{
    ...
    */
    ...
    std::vector<CAmount> vecStandardDenoms = CPrivateSend::GetStandardDenominations();
    bool fSelected = wallets[0]->SelectCoinsByDenominations(nSessionDenom, vecStandardDenoms[vecBits.front()], CPrivateSend::GetMaxPoolAmount(), vecTxDSIn, vCoins, nValueIn, nMinRounds, nMaxRounds);
...

    {
        ...
        for (const auto& txin : vecTxDSIn) {
            wallets[0]->LockCoin(txin.prevout);
        }
    }
        ...
        for (const auto& nBit : vecBits) {
            ...
            while (it2 != vCoins.end()) {
                ...
                if ((*it2).tx->tx->vout[(*it2).i].nValue == nValueDenom) {
                    ...
                    CScript scriptDenom = keyHolderStorage.AddKey(wallets[0].get());
...

    {
        ...
        LOCK(wallets[0]->cs_wallet);
        for (const auto& txin : vecTxDSIn) {
            wallets[0]->UnlockCoin(txin.prevout);
        }
    }
...
bool CPrivateSendClient::MakeCollateralAmounts(CConnman& connman)
{
    ...
    std::vector<std::shared_ptr<CWallet>> wallets = GetWallets();
    if(!wallets[0]->SelectCoinsGrouppedByAddresses(vecTally, false)) {
...
{
    std::vector<std::shared_ptr<CWallet>> wallets = GetWallets();
    LOCK2(cs_main, wallets[0]->cs_wallet);
    ...
    CReserveKey reservekeyCollateral(wallets[0].get());
    // make our change address
    CReserveKey reservekeyChange(wallets[0].get());
    ...
    bool fSuccess = wallets[0]->CreateTransaction(vecSend, wtx, reservekeyChange, nFeeRet, nChangePosRet, strFail, coinControl, true, ONLY_NONDENOMINATED);
    if(!fSuccess) {
        ...
        if(fTryDenominated) {
            ...
            if(!wallets[0]->CreateTransaction(vecSend, wtx, reservekeyChange, nFeeRet, nChangePosRet, strFail, coinControl)) {
    ...
    if(!wallets[0]->CommitTransaction(wtx, {}, {} /* orderForm */, {}, reservekeyChange, &connman, state)) {
...
bool CPrivateSendClient::CreateDenominated(const CompactTallyItem& tallyItem, bool fCreateMixingCollaterals, CConnman& connman)
{
    ...
    if(fCreateMixingCollaterals) {
        CScript scriptCollateral = keyHolderStorageDenom.AddKey(wallets[0].get());
        vecSend.push_back((CRecipient){ scriptCollateral, CPrivateSend::GetMaxCollateralAmount(), false });
    ...
    do {
        ...
        for(auto nDenomValue : vecStandardDenoms) {
            ...
            if(fSkip) {
                ...
                if(nDenomValue != vecStandardDenoms.front() && wallets[0]->CountInputsWithAmount(nDenomValue) > DENOMS_COUNT_MAX) {
            ...
            while(nValueLeft - nDenomValue >= 0 && nOutputs <= 10) {
                CScript scriptDenom = keyHolderStorageDenom.AddKey(wallets[0].get());
